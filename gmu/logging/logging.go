...
func StatsdClient() (*statsd.Client, error) {
	...
	if dogstatsd == nil {
		host := "127.0.0.1:8125"
		if os.Getenv(DataDogAgentEnvName) != "" {
			// N.B. The DataDog ENV variable doesn't appear to include the port.
			host = os.Getenv(DataDogAgentEnvName)
			host = fmt.Sprintf("%v:%v", host, DataDogStatsdPort)
		}
		log.Info("Creating DataDog statsd client", "host", host)
		c, err := statsd.New(host)
		if err != nil {
			log.Error(err, "Failed to create dogstatsd client", "host", host)
			return nil, err
		}
		dogstatsd = c
	}
